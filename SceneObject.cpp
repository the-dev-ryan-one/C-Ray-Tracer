...
glm::vec3 SceneObject::spotLightLighting(glm::vec3 spotLightPos, glm::vec3 spotLightDir,float spotLightAngle, glm::vec3 viewVec, glm::vec3 hit)
{
	float ambientTerm = 0.025;
	float diffuseTerm = 0;
	float specularTerm = 0;
	float spotLightAngleRads = glm::radians(spotLightAngle);
	glm::vec3 normalVec = normal(hit);
	glm::vec3 lightVec = spotLightPos - hit;

	float angle = acos(glm::dot(glm::normalize(-lightVec), glm::normalize(spotLightDir)));

	if (angle > spotLightAngleRads)
	{
		return color_ * ambientTerm;
	}

	return lighting(spotLightPos, viewVec, hit);
